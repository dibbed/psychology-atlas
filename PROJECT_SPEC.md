...
- `ClinicalCase` is the stable identity/slug container; current public title, summary, educational objective, difficulty and primary disorder come from its owned published `current_revision`. Reverse Disorder -> Case study resources and Atlas overview Case counts follow that same public-runnability authority; Dashboard completed-Case history displays/filter semantics are pinned to each attempt revision rather than mutable Case metadata.
...
- SQLite uses Django `transaction_mode=IMMEDIATE`, a 10-second busy timeout and narrowly bounded retry fallback around start/resume and decision advancement. Concurrent identical starts resolve to one shared attempt. Concurrent identical decisions resolve idempotently to one immutable event/answer without double scoring. The final stress regression runs 25 start rounds + 25 decision rounds across five consecutive runs. PostgreSQL continues to rely on row locking plus the database uniqueness invariant.
...
- The focused Case regression is 132/132 PASS and the full backend suite is 189/189 PASS on the final local v0.7.5 code. Final release gates also require Django check, migration drift check, Python compile/pip integrity, SQLite integrity/foreign keys, scientific/research audits, frontend typecheck/build/audit and GitHub CI/CodeQL.
